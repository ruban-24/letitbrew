...
public enum AgentConfigurationInspectionState: Equatable, Sendable { case absent, healthyOwned, repairableOwned, invalid
...
public enum AgentLaunchConnectionPolicy {
 public static func decision(persistedSelection: [String]?, legacyDisconnected: Set<String>, inspections: [AgentConnectionInspection], legacyMigratableAgentIDs: Set<String>) -> AgentLaunchConnectionDecision {
  let supported=Set(AgentID.allCases.map(\.rawValue)); let byID=Dictionary(uniqueKeysWithValues: inspections.map{($0.agentID,$0)})
  let selected: Set<String>
  if let persistedSelection { selected=Set(persistedSelection).intersection(supported) } else { selected=Set(inspections.compactMap { i in legacyMigratableAgentIDs.contains(i.agentID) && (i.state == .healthyOwned || i.state == .repairableOwned) && !legacyDisconnected.contains(i.agentID) ? i.agentID : nil }) }
